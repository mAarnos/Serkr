...
mod test {
    ...
    #[test]
    fn nnf_1() {
        let (f, _) = parse_to_cnf_ast("(P ==> Q) <=> (~Q ==> ~P)").unwrap();
        let (correct_f, _) = parse_to_cnf_ast("(~P \\/ Q \\/ (~Q /\\ P)) /\\ ((P /\\ ~Q) \\/ (Q \\/ ~P))").unwrap();
        assert_eq!(nnf(f), correct_f);                             
    }
          
    #[test]
    fn nnf_2() {
        let (f, _) = parse_to_cnf_ast("~(P <=> Q)").unwrap();
        let (correct_f, _) = parse_to_cnf_ast("(~P \\/ ~Q) /\\ (P \\/ Q)").unwrap();
        assert_eq!(nnf(f), correct_f);
    }
    ...
    fn elim_imp_and_eq_4() {
        let (f, _) = parse_to_cnf_ast("((P ==> Q) /\\ P) <=> Q").unwrap();
        let (correct_f, _) = parse_to_cnf_ast("(((~P \\/ Q) /\\ P) \\/ ~Q) /\\ (~((~P \\/ Q) /\\ P) \\/ Q)").unwrap();
        assert_eq!(elim_imp_and_eq(f, true), correct_f);
    }
    ...
    fn move_nots_inward_not_1() {
        ...
        let (correct_f, _) = parse_to_cnf_ast("P").unwrap();
    ...
    fn move_nots_inward_not_4() {
        let (f, _) = parse_to_cnf_ast("exists x. ~P(x)").unwrap();
        let (correct_f, _) = parse_to_cnf_ast("forall x. P(x)").unwrap();
        assert_eq!(move_nots_inward_not(f), correct_f);
    }
    
    #[test]
    fn move_nots_inward_not_5() {
        let (f, _) = parse_to_cnf_ast("forall x. ~P(x)").unwrap();
