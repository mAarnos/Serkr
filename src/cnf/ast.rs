...
/// If we don't do this, conjuctions of 1000+ clauses cause stack overflows.
/// We do not provide a way to hash the Formula, due to the need for And and Or to be commutative.
/// It would be pretty hard to get the hashing work right.
/// Intensional equality would be so much easier...
#[derive(Debug, Clone)]
#[allow(missing_docs)]
pub enum Formula {
    True,
...
impl PartialEq for Formula {
    fn eq(&self, other: &Formula) -> bool {
        match (self, other) {
            ...
            (&Formula::Predicate(id1, ref args1),
             &Formula::Predicate(id2, ref args2)) => id1 == id2 && args1 == args2,
            (&Formula::And(ref l1), &Formula::And(ref l2)) |
            (&Formula::Or(ref l1), &Formula::Or(ref l2)) => {
                formula_slice_equal_up_to_permutation(l1, l2)
            }
            (&Formula::Not(ref f1), &Formula::Not(ref f2)) => f1 == f2,
            (&Formula::Implies(ref f_l_1, ref f_r_1),
             &Formula::Implies(ref f_l_2, ref f_r_2)) => f_l_1 == f_l_2 && f_r_1 == f_r_2,
            (&Formula::Equivalent(ref f_l_1, ref f_r_1),
             &Formula::Equivalent(ref f_l_2, ref f_r_2)) => {
                (f_l_1 == f_l_2 && f_r_1 == f_r_2) || (f_l_1 == f_r_2 && f_r_1 == f_l_2)
            }
            (&Formula::Forall(id1, ref f1), &Formula::Forall(id2, ref f2)) |
            (&Formula::Exists(id1, ref f1), &Formula::Exists(id2, ref f2)) => {
                id1 == id2 && f1 == f2
            }
            (_, _) => false,
        }
